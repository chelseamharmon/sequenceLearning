...
for thisTrial_3 in trials_3:
    ...
    for ii in range(len(minImages)):
        ...
        while continueRoutine:
            ...
            if key_resp_9.status == STARTED:
                ...
                if len(theseKeys) > 0:  # at least one key was pressed
                    ...
                    # was this 'correct'?
                    #if (theseKeys) == (correctKey):
                    #print(key_resp_9.rt[-1], ' is the keyresp! and goTime is ',goTime )
                    #print((key_resp_9.rt[-1] > goTime))
                    #if ((theseKeys) == (['1']) and (key_resp_9.rt[-1] > goTime) ): 
                    if ((theseKeys) == (correctResponses[ii]) and (key_resp_9.rt[-1] > goTime) ):
                        trials_3.addData('stimulusPresent.t',goTime)
                    #if (key_resp_9.keys == str(u'correctKey')) or (key_resp_9.keys == u'correctKey'):
                        #if len(key_resp_9.keys) == 1:
                        if (key_resp_9.keys[0] == (correctResponsesKeys[ii])):                       
                            key_resp_9_corr = 1
                            trialCorrect = 1
                            trials_3.addData('trialCorrect',trialCorrect)
                            print('trialcorrect: ', trialCorrect)
                            print('rt is ', key_resp_9.rt, '  t is: ', t) 
                        else: 
                            key_resp_9_corr = 0
                            trialCorrect = 0
                            trials_3.addData('trialCorrect',trialCorrect)
        ...
        thisExp.nextEntry()
    jj = jj + 1 
    print('rep number at end of 3 trials ', jj)
    if(jj == 11):
        break 

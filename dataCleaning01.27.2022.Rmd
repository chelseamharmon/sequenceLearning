...
length(all.use.these.data.files)
...
length(taskData$participant)
both.waves.file.path.name <- both.waves.file.path.name[-1]
all.use.these.data.files <- all.use.these.data.files[-1]
both.waves.file.path.name[242]
taskData3 <- read.csv((both.waves.file.path.name[242]), stringsAsFactors = F)
length(taskData3$participant)
...
length(both.waves.file.path.name)
both.waves.file.path.name
2:length(both.waves.file.path.name)
length(taskData$participant)

length(both.waves.file.path.name)
i=2
both.waves.file.path.name[1]
both.waves.file.path.name[1]

for(i in 2:length(both.waves.file.path.name)){
...
#formerly (with extra PA003 data doulbes (off by 1 error) )
...
#new 
write.csv(taskData, file= "all.subject.task.data_07.18.22.csv", row.names = F)
taskData <- read.csv("all.subject.task.data_07.18.22.csv")
...
unique(taskData[taskData$participant=="3",]$date)

length(taskData[taskData$participant=="3",]$date)

table(taskData$participant)


unique(taskData[taskData$participant=="6",]$date)

unique(taskData[taskData$participant=="61",]$date)
length(taskData[taskData$participant=="61",]$date)
...
taskData <- taskData[!taskData$participant=="89",]
...
length(unique(taskDataTouch$participant))
length(unique(taskData$participant))
head(taskDataTouch)
...
summaryRT <- taskData %>% 
  group_by(Group) %>% 
  summarise_at(vars(responseTimeCorrect), funs(mean(., na.rm=TRUE), sd(., na.rm=T))) 
head(summaryRT)


ggplot(data=summaryRT, aes(Group, mean)) +
  geom_point()
...
class(allTaskData$responseTimeCorrect)

allTaskData$responseTimeCorrect <- as.numeric(allTaskData$responseTimeCorrect)

summaryRT <- allTaskData %>% 
  group_by(participant, Block) %>% 
  summarise_at(vars(responseTimeCorrect), funs(mean(., na.rm=TRUE), sd(., na.rm=T))) %>% 
  merge(allTaskData, .,)
head(summaryRT)

ggplot(data=summaryRT, aes(Block, mean)) +
  geom_point()

allTaskData$trialIncorrectCoded <- ifelse(is.na(allTaskData$responseTimeIncorrect), 0, 1)


summaryRT <- allTaskData %>% 
  filter(trialIncorrectCoded==1) %>%
  group_by(participant, Block) %>% 
  summarise_at(vars(responseTimeCorrect), funs(mean(., na.rm=TRUE), sd(., na.rm=T))) %>% 
  merge(allTaskData, .,)
head(summaryRT)
...
#Moved this to above! 
#Removing additional subject after realizing they compelted incorrect version of task 
```{r}
# taskData <- read.csv("allTaskDataCleaned_withDemo1.28.2022.csv")
# unique(taskData$participant)
# length(taskData)
# length(taskData$participant) #85760
# 
# taskData <- taskData[!taskData$participant==89,]
# length(taskData$participant)
# length(taskData)
# 
# 85760-85440
# write.csv(taskData, "allTaskDataCleaned6.22.2022.csv", row.names=F)

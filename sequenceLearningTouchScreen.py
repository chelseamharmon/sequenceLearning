...
im = visual.ImageStim(win, image='images/minion.png')
im.size=(0.45,0.79)
im_positions = [(-0.75, -0.04), (0.25, -0.04), (-0.25, -0.04), (0.75, -0.04), (0.75, -0.04), (-0.25, -0.04), (0.25, -0.04), (0.75, -0.04)]
...
for thisTrial_3 in trials_3:
    ...
    for ii in range(len(minImages)):
        ...
        trials_3.addData('correctLocation',im_positions[ii])
        trials_3.addData('correctImage',minImages[ii])
        trials_3.addData('repetitionNumber',jj)
        # update component parameters for each repeat
        pos1.setImage(minImages[ii])
        im.pos=(im_positions[ii])
        key_resp_9 = event.BuilderKeyResponse()
        # keep track of which components have finished
        stim1Components = [pos1, key_resp_9, im.pos]
...
#                if np.any(np.array(buttons)):
                if mouse.mouseMoved(): 
                    if ((im.contains(mouse)) and (times > pos1.tStart)):
#                    if ((mouse.isPressedIn(im)) and (times > pos1.tStart)):
                        trials_3.addData('stimulusPresent.t',pos1.tStart)
                        trials_3.addData('responseTimeCorrect', times[0]) #Note this is the raw time - including time for incorrect trials
                        trials_3.addData('pressLoc',pos)
                        if times[0] <= pos1.tStart:
                            antic = 1 
                            trials_3.addData('anticipatoryResponse',antic) #Anticipatory responses only recorded if aniticipating with correct location
                        continueRoutine = False
                    else: 
                        trials_3.addData('responseTimeIncorrect', times[0])
                        trialInCorrect = 1
                        trials_3.addData('trialIncorrect',trialInCorrect)
                else:
                    continueRoutine = True
...
minImages = ['images/min1.png', 'images/min2.png', 'images/min3.png', 'images/min4.png']

im = visual.ImageStim(win, image='images/minion.png')
im.size=(0.45,0.79)
im_positions = [(-0.75, 0.04), (-0.25, 0.04), (0.25, 0.04), (0.75, 0.04)]
...
for thisTrial_2 in trials_2:
    ...
    for ii in range(len(im_positions)):
        ii = randint(0,4)
        ...
        trials_3.addData('repetitionNumber',jj)
        # keep track of which components have finished
        randomStimComponents = [image, key_resp_9, im.pos]
        ...
        while continueRoutine:
            ...
            if key_resp_9.status == STARTED:
                ...
                if mouse.mouseMoved(): 
                    if ((im.contains(mouse)) and (times > pos1.tStart)):
                        trials_3.addData('stimulusPresent.t',image.tStart)
                        trials_3.addData('responseTimeCorrect', times[0]) #Note this is the raw time - including time for incorrect trials
                        trials_3.addData('pressLoc',pos)
                        if times[0] <= image.tStart:
                            antic = 1 
                            trials_3.addData('anticipatoryResponse',antic) #Anticipatory responses only recorded if aniticipating with correct location
                        continueRoutine = False
                    else: 
